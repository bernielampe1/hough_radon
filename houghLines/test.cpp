...
class HoughSpace
{
  ...
  public:
         ...
         accumulator(0)
    {
      ...
      memset(this->accumulator, 0,
             this->numAngs * this->numDists * sizeof(int));
    }
      ...
      const string &fname)
    {
      ...
      if (!ofile) { throw Exception("could not open file for writing"); }
      ...
      for(int a = 0; a < this->numAngs; a++)
      {
        ...
        {
          nvotes = (unsigned char)(this->accumulator[a * this->numDists + d] *
                                   scale + 0.5);
