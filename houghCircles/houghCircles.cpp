...
void incrementPixel(const int &rows, const int &cols, int *img, const int &r,
                    const int &c) {
...
void incrementCircle(const int &x0, const int &y0, const int &r,
                     const int &rows, const int &cols, int *img) {
  int f = 1 - r;
  int ddF_x = 1;
  int ddF_y = -2 * r;
  int x = 0;
  ...
  rp = y0 + r;
  cp = x0;
  incrementPixel(rows, cols, img, rp, cp);
...
public:
  HoughSpace(const int minRad, const int maxRad, const int minC, const int maxC,
  ...
  void addVotes(const int r, const int c) {
    for (int rad = 0; rad < numRadii; rad++) {
      incrementCircle(c, r, rad + minRadius, numRows, numCols, acc[rad]);
    }
  }
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    HoughSpace acc(3, 50, 0, numCols - 1, 0, numRows - 1);
